...
## OverallQual
```{r OverallQual_visual, fig.height = 5, fig.width = 8}
...
類別變數被我從43個減少到28個、數值變數則從38個增加到52個，比較有利於後續模型建模。
...
從相關性檢測可以證明我的推測沒錯，1stFlrSF+2ndFlrSF和GrLivArea相關性等於1(其實也是廢話XD)。其實我本來以為GrLivArea只包含1樓，但是看了分佈圖與相關性圖後才發現還包含2樓。
其他像是TotRmsAbvGrd和GrLivArea高相關性也很合理，坪數越大房間越多嘛。
...
all %>% 
  filter(!(is.na(SalePrice))) %>% 
  ggplot( aes(x=NeighRich, y=SalePrice)) +
  geom_bar(stat='summary', fun.y = "median", fill='dodgerblue2') +
  geom_bar(stat='summary', fun.y = "sd", fill='deeppink3',width = 0.5) +
  scale_y_continuous(labels = comma) + 
  geom_label(stat = "count",aes(label = ..count..,y = ..count..)) + 
